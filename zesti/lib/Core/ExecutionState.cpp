...
ExecutionState::ExecutionState(KFunction *kf) 
  : fakeState(false),
    ...
    symbexEnabled(false),
    seedingTTL(0),
    seedingInstExecuted(0),
    lastInstructionGEP(false),
    markForDeletion(false),
    branchTime(NULL),
    inPatch(false),
    nextForkInterleaved(false)
{
  pushFrame(0, kf);
}
...
ExecutionState::ExecutionState(const ExecutionState& state)
  : fnAliases(state.fnAliases),
    ...
    symbexEnabled(state.symbexEnabled),
    seedingTTL(state.seedingTTL),
    ...
    branchTime(0),
    nextForkInterleaved(state.nextForkInterleaved),
