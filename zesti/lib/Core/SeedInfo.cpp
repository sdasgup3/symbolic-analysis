...
void SeedInfo::patchSeed(const ExecutionState &state, 
  ...
  std::vector< ref<ReadExpr> > reads, reads2;
  ...
  bool res;
  bool success = solver->mayBeTrue(state, assignment.evaluate(condition), res, false);
  assert(success && "FIXME: Unhandled solver failure");
  (void) success;
  if (res) {
    // thorough check
    // add constraints one at a time because some solvers may expect
    // a SAT set of path conditions
    std::vector<ref<Expr> > simplified;
    bool failed = false;
    ConstraintManager::constraint_iterator it, ite;
    for (it = tmp.constraints.begin(), ite = tmp.constraints.end();
          it != ite; ++it) {
      ref<Expr> cond = assignment.evaluate(*it);
      if (isa<ConstantExpr>(cond)) {
        if (cast<ConstantExpr>(cond)->isFalse()) {
          failed = true;
          break;
        }
      } else {
        ExecutionState tmp2(simplified);
        bool success = solver->mayBeTrue(tmp2, cond, res, false);
        assert(success && "FIXME: Unhandled solver failure");
        (void) success;
        if (res) {
          simplified.push_back(cond);
        } else {
          failed = true;
          break;
        }
