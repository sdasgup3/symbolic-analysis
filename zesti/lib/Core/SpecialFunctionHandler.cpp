...
void SpecialFunctionHandler::handleAssume(ExecutionState &state,
  ...
  bool res;
  bool success = executor.solver->mustBeFalse(state, e, res, false);
...
void SpecialFunctionHandler::handleMakeSymbolic(ExecutionState &state,
  ...
  for (Executor::ExactResolutionList::iterator it = rl.begin(), 
    ...
    if (res) {
      ...
      std::cerr << "wrong size given to klee_make_symbolic: " << arguments[1] <<
        " (given), " << mo->getSizeExpr()  << " (expected)" << std::endl; 
      executor.terminateStateOnError(*s, 
...
void SpecialFunctionHandler::handleEnableSeeding(ExecutionState &state,
                                              KInstruction *target,
                                              std::vector<ref<Expr> > &arguments) {
  unsigned TTL = 2;
  bool interleave = 0;
...
void SpecialFunctionHandler::handleDisableSeeding(ExecutionState &state,
                                               KInstruction *target,
                                               std::vector<ref<Expr> > &arguments) {
...
void SpecialFunctionHandler::handleSkipChecks(ExecutionState &state,
                                              KInstruction *target,
                                              std::vector<ref<Expr> > &arguments) {
  ZestSkipChecks = true;
}

void SpecialFunctionHandler::handleDoChecks(ExecutionState &state,
                                               KInstruction *target,
                                               std::vector<ref<Expr> > &arguments) {
...
void SpecialFunctionHandler::handlePatchBegin(ExecutionState &state,
                                              KInstruction *target,
                                              std::vector<ref<Expr> > &arguments) {
  assert(arguments.size()==0 &&
...
void SpecialFunctionHandler::handlePatchEnd(ExecutionState &state,
                                              KInstruction *target,
                                              std::vector<ref<Expr> > &arguments) {
