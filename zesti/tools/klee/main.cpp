...
namespace {
  ...
  enum TestOutputLevel {
    OutputAll, OutputErrors, OutputNone
  };

  cl::opt<TestOutputLevel>
  OutputLevel("output-level", 
       cl::desc("Chose the output level for test cases (all by default)."),
       cl::values(clEnumValN(OutputAll, "all", "Create ktest files for all paths"),
                  clEnumValN(OutputErrors, "error", "Create ktest files only for errors"),
...
KleeHandler::KleeHandler(int argc, char **argv) 
  ...
  if (OutputDir=="") {
    ...
    for (int i = 0; ; i++) {
      ...
      if (lstat(theDir.c_str(), &foostat) == 0 || errno != ENOENT) {
        //closedir(dir);
      } else {
        break;
      }
  ...
  // some programs may change the umask. make sure we have appropriate permissions
  if (chmod(m_outputDirectory, 0755) < 0) {
    perror("chmod");
    assert(false);
    exit(1);
  }
...
static llvm::Module* MakeArgsSymbolic(llvm::Module *mainModule)
{
	...
	if (userMain && userMain->arg_size() >= 2) {
		llvm::Constant *fc = mainModule->getOrInsertFunction("klee_mark_arg_symbolic",
					 Type::getVoidTy(getGlobalContext()),
				 Type::getInt32Ty(getGlobalContext()),
				 userMain->getFunctionType()->getParamType(1),
					 NULL);
		...
		if (makeSymbolic) {
			...
			args.push_back(++userMain->arg_begin());
			  ...
			  CallInst::Create(makeSymbolic, args, "", userMain->begin()->begin());
                        #else
			  CallInst::Create(makeSymbolic, args.begin(), args.end(), "", userMain->begin()->begin());
                        #endif  
		} else {
...
static llvm::Module* RenameUclibcFunctions(llvm::Module *m)
{
  Function* f;
  GlobalAlias* ga;
  std::vector<std::string> conflicting(conflictingUclibc,
                                    conflictingUclibc+NELEMS(conflictingUclibc));
  
  std::vector<std::string>::const_iterator it, ite;
  for (it = conflicting.begin(), ite = conflicting.end();
       it != ite; ++it)
  {
    f = m->getFunction(*it);
    if (!f)
...
// This is a hack to make the first open() call in a user program return 3 (see test/Feature/FDNumbers.c).  Should call closeFirstFDs() afterwards.
...
int main(int argc, char **argv, char **envp) {  
  ...
  if (WithPOSIXRuntime) {
    ...
    SmallString<128> LibraryDir;
      ...
      LibraryDir = KLEE_INSTALL_LIB_DIR ;
    #else
      LibraryDir = KLEE_DIR;
      llvm::sys::path::append(LibraryDir,RUNTIME_CONFIGURATION);
      llvm::sys::path::append(LibraryDir,"lib");
    #endif
  #else  
    #if defined(KLEE_LIB_DIR) && defined(USE_KLEE_LIB_DIR)
