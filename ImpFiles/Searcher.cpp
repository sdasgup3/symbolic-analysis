  ...
  , sensitiveInst(s)
{
  ...
  //klee_message("DSAND: ZESTSearcher: PatchCheckBefore:%d SearchUntil:%d distance %d", PatchCheckBefore, (int)SearchUntil, distance);
}
/*
...
ExecutionState &ZESTSearcher::selectState() {
  ...
  do {
    state = states.back(); // This initial back gives null pointer, 
                            // the front has a state
    /*
    if(state) {
      klee_message("DSAND: Inside Searcher inst ");
      for (std::list<ExecutionState*>::iterator itn = states.begin(), ien = states.end(); itn != ien; ++itn) {
        ExecutionState* pState = *itn;
        if(pState) {
          llvm::errs() << *(pState->pc->inst) << "\n";
        }
      }
      klee_message("DSAND: End");
    } else {
      klee_message("DSAND: What is it");
    }
    */
    if (state) {
      //klee_message("DSAND: What is it2");
      ...
      if (1 == state->seedingTTL &&
        ...
        //llvm::errs() << "DSAND: suspended" << *(state->pc->inst) << "\n";
      } else {
        if (dfsStage && DiscardFarStates) {
          state->markForDeletion = 1;
          //std::cerr << "Marking del state " << state << std::endl;
        }
        return *state;
      }
  ...
  if (state) {
    ...
    /*
    klee_message("if no state is found consume the rest of the states directly");
      for (std::list<ExecutionState*>::iterator itn = states.begin(), ien = states.end(); itn != ien; ++itn) {
        ExecutionState* pState = *itn;
        if(pState) {
          llvm::errs() << *(pState->pc->inst) << "\n";
